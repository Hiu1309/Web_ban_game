...
if (!$supplier_result || $supplier_result->num_rows == 0) {
    ...
    $insert_supplier = "INSERT INTO supplier (SupplierName, Email, Phone, Address) VALUES ('Nhà cung cấp mặc định', 'supplier@example.com', '0123456789', 'Địa chỉ mặc định')";
    ...
    if(isset($_FILES["productImg"]) && $_FILES["productImg"]["error"] == 0) {
        ...
        if ($uploadOk == 1) {
            ...
            if (move_uploaded_file($_FILES["productImg"]["tmp_name"], $target_file)) {
                ...
                $productImg = "/Web_ban_game/Assets/Images/Game/" . $newFileName;
                
                // Debug - hiển thị đường dẫn hình ảnh
                echo "<script>console.log('Image path: " . $productImg . "');</script>";
                
                $sql = "INSERT INTO product (ProductName, ProductImg, Author, Publisher, Quantity, Price, Description, SupplierID, Status) 
                        VALUES ('$productName', '$productImg', '$author', '$publisher', $quantity, $price, '$description', $supplierID, $status)";
                        ...
                        <tbody>
                            ...
                            if ($result && $result->num_rows > 0) {
                                while($row = $result->fetch_assoc()) {
                                    ...
                                    // Đã xóa phần nút thao tác ở đây
                                    echo "</tr>";
                                }
                            } else {
